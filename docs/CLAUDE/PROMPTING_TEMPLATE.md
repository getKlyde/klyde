# PROMPTING_TEMPLATE.md — Resume klyd development fast
...
```
I'm building klyd — a decision memory harness for terminal coding agents (aider, opencode).
...
```
I'm building klyd. Working on TASK-[N].
...
```
I'm building klyd — a git-hook-based architectural decision memory layer for aider/opencode.
...
Key files:
- klyd/db.py — SQLite interface (decisions table, store/get/reinforce/flag/archive)
- klyd/extractor.py — LLM extraction call
- klyd/injector.py — injection formatting
- klyd/cli.py — Click CLI (init, config, status, review, run, extract-commit, prepare-injection)
- hooks/post-commit.sh — calls `klyd extract-commit`
- hooks/pre-commit.sh — calls `klyd prepare-injection`
...
```
I'm building klyd (decision memory harness for coding agents).
