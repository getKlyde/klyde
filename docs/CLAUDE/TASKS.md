# TASKS.md — Gated build tasks for klyd
...
├── hooks/
│   ├── post-commit.sh  # POSIX shell, calls klyd CLI
│   └── pre-commit.sh   # POSIX shell, calls klyd CLI
├── schema/
...
**Gate:** `python -m klyd --help` runs without error and shows a help message.

---

## TASK-02 · `klyd init` installs git hooks

**What:** Implement `klyd init`. It must:
...
- Create `.klyd/` directory
- Write `.klyd/config.json` with empty defaults
- Print confirmation

Hooks do nothing yet except `echo "klyd: [hook-name] fired"`.

**Gate:**
1. Run `klyd init` in a test git repo
2. Make a commit
3. See `klyd: post-commit fired` in terminal output
4. Stage a file, see `klyd: pre-commit fired`
...
## TASK-04 · `klyd status` renders memory store

**What:** Implement `klyd status` CLI command.
...
**What:** Implement `klyd config` and `extractor.py`.

- `klyd config --api-key sk-...` → writes to `.klyd/config.json`
- `klyd config --model claude-sonnet-4-6` → writes model preference (default: claude-sonnet-4-6)
...
4. Invalid API key → error is caught, logged to `.klyd/errors.log`, hook exits 0 (never block commits)
...
Flow:
1. `post-commit.sh` calls `klyd extract-commit`
2. `klyd extract-commit`:
   ...
   - If API error → log to `.klyd/errors.log`, exit silently

**Gate:**
1. Make a commit with clear JWT auth introduction → `klyd status` shows new HIGH-confidence decision
2. Make a second commit that reinforces same decision → reinforcement_count increments
3. Make a commit that contradicts → flagged decision appears in NEEDS REVIEW in `klyd status`
4. Intentionally break API key → commit completes normally, error in `.klyd/errors.log`
...
Flow:
1. `pre-commit.sh` calls `klyd prepare-injection`
2. `klyd prepare-injection`:
   ...
   - If decisions found: writes `.klyd/injection.txt` with formatted message
   - If no decisions: writes empty `.klyd/injection.txt`
3. `klyd run aider` — before starting aider, reads `.klyd/injection.txt` and prepends to initial system context or first user message
...
3. Check `.klyd/injection.txt` — all 3 decisions present, correct format
...
## TASK-08 · `klyd review` interactive resolution

**What:** Implement `klyd review` for human-in-the-loop conflict resolution.
...
2. `klyd review` surfaces it correctly
3. Accept → old decision archived, new one active in `klyd status`
4. Reject → new finding archived, old one unchanged
5. After resolution, `klyd status` shows 0 items in NEEDS REVIEW

---

## TASK-09 · End-to-end test on a real project

**What:** Install klyd on a real project (not a test fixture) and run a full cycle.

Use a small real project (a Python CLI or Node API you're building anyway).

Steps:
1. `klyd init` + `klyd config --api-key ...`
2. Make 5 commits with clear architectural decisions
3. `klyd status` — verify correct decisions captured
4. Make a contradicting commit
5. `klyd review` — resolve it
6. `klyd run aider` — make aider do something that touches governed modules
7. Verify aider does not contradict injected decisions without being told to

**Gate:**
The agent demonstrably behaves differently (more architecturally consistent) with klyd active than without. This is qualitative but real. If you can't tell the difference, the injection isn't working.

---

## TASK-10 · Package + README + release

**What:** Make klyd installable by someone who wasn't in this conversation.

- `pyproject.toml` with correct entry point: `klyd = klyd.cli:main`
- `pip install klyd-harness` (or whatever name is available on PyPI)
...
- `.klyd/` added to `.gitignore` template (don't commit the memory store)

**Gate:**
Fresh machine, fresh virtual env. `pip install klyd-harness`, `klyd init`, `klyd config`, `klyd run aider` — works in under 5 minutes. Decisions populate after 3 commits. `klyd status` shows correct state.
